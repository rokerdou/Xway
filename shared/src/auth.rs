...
impl AuthPacket {
    ...
    pub fn new_with_ip(
        ...
        let hmac =
            Self::compute_hmac_with_ip(&username, timestamp, sequence, shared_secret, &client_ip);
    ...
    pub fn verify(&self, shared_secret: &[u8], max_time_diff_secs: u64) -> Result<()> {
        ...
        let expected_hmac = if self.client_ip == "0.0.0.0" {
            // 向后兼容：使用旧的HMAC计算方法
            Self::compute_hmac(&self.username, self.timestamp, self.sequence, shared_secret)
        } else {
            // 新版本：包含客户端IP的HMAC
